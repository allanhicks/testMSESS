...
runMSE <- function(continue         = T,
  ...
  if(exists("printDebug")){if(printDebug) {print("Debug statements will be printed because on object in your working directory named printDebug is TRUE")}}
  ...
  scenarios <- runScenarios(scenarioInfo = scenarioInfo,
                            ...
                            emailPassword    = emailPassword)
  if(exists("printDebug")){if(printDebug) {print("Finished runScenarios")}}
...
runSingleAssessment <- function(simDat,
  ...
  # Stip out the first 5 columns, which are "run mceval Yr Fleet Sex" and make numeric
  simDat$assessSelexFishery <- as.numeric(selex1[-(1:5)]) # 1 = Fishery
  simDat$assessSelexSurvey  <- as.numeric(selex2[-(1:5)]) # 2 = Survey
...
mergeScenario <- function(simDir,
  ...
  if(assessmentSim){
    ...
    ageLabels                         <- paste("age",seq(0,sims$maxA),sep="")
    sims$assessSelexFishery           <- as.table(sims$assessSelexFishery)
    colnames(sims$assessSelexFishery) <- ageLabels
    rownames(sims$assessSelexFishery) <- simLabels
    sims$assessSelexFishery           <- sortDataFrameByRowName(sims$assessSelexFishery)

    sims$assessSelexSurvey            <- as.table(sims$assessSelexSurvey)
    colnames(sims$assessSelexSurvey)  <- ageLabels
    rownames(sims$assessSelexSurvey)  <- simLabels
    sims$assessSelexSurvey            <- sortDataFrameByRowName(sims$assessSelexSurvey)
...
cleanScenarioDir <- function(fileList=c(.DATA_FILE,
                                        .SIM_CONTROL_FILE,
                                        .FORECAST_FILE,
  ...
  # keep the .ASSESS_CONTROL_FILE and the .SIMULATIONS_DIR_NAME
  # directory.
  filesToRemove <- file.path(dirName,fileList)
...
if(exists("scen")) {  #do this only if sourcing the file from a cmd prompt with scen set
    runMSE(continue=T, useSystem=F, verbose=F, sendEmailUpdates=F, scen=scen, emailAddress="yourname@gmail.com", emailPassword="")
    #mergeMSE()
}
