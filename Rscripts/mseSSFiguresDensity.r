...
plotDensity <- function(scenarios,
  ...
  for(scenario in 1:length(scenarios)){
    # Check to see that the years requested to be plotted are all present in the assessed data

    if(!(all(years %in% scenarios[[1]]$assessYears))){
      return(FALSE)
    }
...
makePolys <- function(scenarios,
  ...
  legend("topright",legendList,col=legendBorderCols,fill=legendBorderCols,bty="n")
}
