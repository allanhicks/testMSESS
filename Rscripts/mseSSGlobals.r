...
.SS_FILES_PREFIX       <- "ssFiles"         # Prefix for the RDATA files written after each simulation run.
...
.FIRST_YEAR_BIAS_CORR  <- "First year of full bias correction"        # used in modifyBiasCorr()
...
.LAST_YEAR_NOBIAS      <- "Last recruit dev with no bias_adjustment"  # used in modifyBiasCorr()
.MAX_BIAS_ADJ          <- "Maximum bias adjustment in MPD"            # used in modifyBiasCorr()
.MCEVAL                <- "-mceval"
.NOHESS                <- "-nohess"
...
.NUM_AGE_ERR_DEFS      <- 41    #this needs to be fixed.  this really refers to the ageing error def that has no cohort effect in it.  There should be a better way to not make a mistake.
...
.TOP_OF_FISHERY_SELEX  <- "Fishery age-based"
...
.REP_FILE              <- "Report.sso"
.NEW_PSV_FILE          <- "new.psv"
.ADD_PSV_FILE          <- "newss3.psv"
.ADD_ADMODEL_FILE      <- "newadmodel.cov"
