...
plotActualvsPerceived <- function(scenarios,
  ...
  noFishingScenario <- NULL
  perfectInformationScenario <- NULL
  ...
  goodToGo <- TRUE
  if(is.null(noFishingScenario)){
    cat("plotActualvsPerceived: Error, no 'NoFishing' scenario found.\n")
    gottToGo <- FALSE
  }
  if(is.null(perfectInformationScenario)){
    cat("plotActualvsPerceived: Error, no 'PerfectInformation' scenario found.\n")
    goodToGo <- FALSE
  }
  if(!goodToGo){
    return(FALSE)
